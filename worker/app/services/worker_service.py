...
class WorkerService:
    ...
    async def process_download_entry(self, entry: Entry, entry_service: EntryService):
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Failed to download entry {entry.id} from URL: {entry.source_url}",
                )
    ...
    async def process_asr_entry(self, entry: Entry, entry_service: EntryService):
        ...
        try:
            ...
            try:
                is_valid, validation_error = self.asr_service.validate_audio_file(
                    entry.file_path,
                )
                if not is_valid:
                    ...
                    if is_size_error:
                        logger.info(
                            f"Entry {entry.id}: Large file detected, will use chunking for processing: {validation_error}",
                        )
            ...
            if success and transcript:
                ...
                logger.info(
                    f"Entry {entry.id}: Transcription successful, saving transcript ({len(transcript)} characters)",
                )
                await entry_service.update_entry_transcript(
                    entry.id,
                    transcript,
                    words,
                    segments,
                ...
                if not success:
                    logger.warning(
                        f"Entry {entry.id}: Transcription marked as unsuccessful",
                    )
                if not transcript:
                ...
                if error_msg and self.asr_service.is_permanent_error(error_msg):
                    ...
                    await entry_service.update_entry_status(
                        ...
                        EntryStatus.ERROR,
                        error_message=error_msg
                        or "Transcription failed (permanent error)",
                    )
                    logger.error(
                        f"Entry {entry.id} permanent transcription error: {error_msg}",
                    )
                else:
                    ...
                    logger.warning(
                        f"Entry {entry.id} temporary transcription error: {error_msg}",
                    )
    ...
    async def process_url_download(
        ...
        if success and result:
            ...
            logger.info(
                f"Entry {entry.id} download completed and uploaded to S3: {s3_key}",
            )
            return True
            ...
            if error_msg and self.download_service.is_permanent_error(error_msg):
                ...
                if self.download_service.is_youtube_auth_error(error_msg):
                    ...
                    logger.warning(
                        f"Entry {entry.id} YouTube authentication required: {error_msg}",
                    )
                else:
                    ...
                    logger.error(
                        f"Entry {entry.id} permanent download error: {error_msg}",
                    )
            else:
                # Temporary error - mark as NEW for retry
                await entry_service.update_entry_status(
                ...
                logger.warning(
                    f"Entry {entry.id} temporary download error: {error_msg}",
                )
